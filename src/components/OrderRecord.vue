<template>
    <div class="contract_record_div">
        <el-tabs class="transtionBar" v-model="activeIndex" @tab-click="handleIndex">
            <el-tab-pane :label="$t('contract.position')" name="0"></el-tab-pane>
        ...
        <el-table class="fishTable" :data="tableData" :empty-text="$t('tip.noRecord')">
            ...
            <el-table-column width="60" prop="leverName" :label="$t('contract.lever')"></el-table-column>
            <!-- v-if="activeIndex == '0'" -->
            <!-- 盈亏 -->
            ...
            <el-table-column prop="tradePrice" v-if="activeIndex == '0'  || activeIndex == '2' " :label="$t('contract.average')"></el-table-column>
            <el-table-column prop="unit" v-if="activeIndex == '0'" :label="$t('contract.current')"></el-table-column>
            <el-table-column prop="exitPrice" v-if="activeIndex == '2'" :label="$t('transaction.averagePrice')">

            </el-table-column>
            <el-table-column prop="totalPrice" :label="$t('contract.total')"></el-table-column>
            <el-table-column prop="numbers" :label="$t('contract.num')"></el-table-column>
            <el-table-column prop="fee" :label="$t('contract.fee')"></el-table-column>
        ...
        <el-dialog 
            ...
            <div class="average_content">
                ...
                <ul>
                    ...
                    <li>
                        <!-- 当前价 -->
                        <span>{{$t('contract.current')}}</span>
                        <span>{{Number(averageContent.unit).toFixed(2)}}</span>
                    </li>
                    <li>
                        <!-- 手续费 -->
                        <span>{{$t('contract.fee')}}</span>
